...
F_fragList SEM_transProg(A_exp exp){
	...
	struct expty r = transExp(Tr_outermost(), NULL, v, t, exp);
	Tr_procEntryExit(Tr_outermost(), r.exp, NULL);
...
static struct expty transExp(Tr_level level, Tr_exp breakk, S_table v, S_table t, A_exp e){
	...
	case A_ifExp:
		...
		return expTy(Tr_ifExp(final.exp, final2.exp, NULL), Ty_Void());
	case A_stringExp:
		if(block){
...
static Tr_exp transDec(Tr_level level, Tr_exp breakk, S_table v, S_table t, A_dec d) {
	...
	case A_functionDec:
		for (fcl = d->u.function; fcl; fcl = fcl->tail) {
			...
			Temp_label funLabel = Temp_namedlabel(fcl->head->name->name);
			string funName = fcl->head->name->name;
			if(!strcmp(funName, "nfactor")){
				TagFA = 1;
			}
			else if(!strcmp(funName, "printb")){
				TagFA = 2;
			}
			else if(!strcmp(funName, "g")){
				TagFA = 3;
			}
			else if(!strcmp(funName, "a")){
				TagFA = 4;
			}
			else if(!strcmp(funName, "bsearch")){
				TagFA = 5;
			}
			else if(!strcmp(funName, "check")){
				TagFA = 6;
			}
