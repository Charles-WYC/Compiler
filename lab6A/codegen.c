...
static Temp_temp munchExp(T_exp e)
{
    ...
    switch (e->kind) {
        ...
        case T_CALL: 
        {
            ...
            emit(AS_Oper(String("call `s0"), F_calldefs(), Temp_TempList(r, munchArgs(0, e->u.CALL.args)), NULL));
...
static void munchStm(T_stm s)
{
	...
	switch (s->kind) {
        ...
        {
            munchExp(s->u.EXP); 
            emit(AS_Move(String("mov `s0, `d0"), Temp_TempList(munchExp(s->u.EXP), NULL), Temp_TempList(munchExp(s->u.EXP), NULL)));	
            break;
        }
